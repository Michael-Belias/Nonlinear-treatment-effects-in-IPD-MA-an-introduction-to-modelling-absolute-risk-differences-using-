...
### RCS  
### Define the knots position in 5%, 27.5%,  50%, 72.5% and 95% quantiles of BMI
Knots.RCS.df3.with.pseudo =   list(BMI=quantile(df3.with.pseudo$BMI, probs = c(0.05,0.275,0.5,0.725,0.95))) 
...
fit.RCS.df3.with.pseudo = gam( formula =formula.RCS,
                   knots = Knots.RCS.df3,
                   family = binomial("logit"), weight= weight,
...
### B-splines with pseudo
###

## The formula for all studies is the same so we save it 

formula.BS.pseudo = Y~ BMI + Treatment+ s(BMI,bs ="bs",fx=T,by = Treatment,k = 5,m=c(2,0))

## Number of studies

nstudies.BS.df3.pseudo = length(unique(df3.with.pseudo$Study))


### Fit a stacked analysis in the whole data-set to get the model matrix to get the predicted outcomes later on

fit.BS.df3.pseudo = gam( formula =formula.BS.pseudo,
                  family = binomial("logit"), 
                  data = df3.with.pseudo)

Knots.BS.df3.pseudo =  list(BMI =fit.BS.df3.pseudo$smooth[[1]]$knots)

### Get the model matrices for each data-set

X.p.BS.df3.pseudo =  model.matrix(fit.BS.df3.pseudo)

### Create empty matrix for the estimated splines coefficients

estimated.coefficients.BS.df3.pseudo = matrix(NA,
                                       ncol = length(fit.BS.df3.pseudo$coefficients),nrow=nstudies.BS.df3.pseudo,
                                       dimnames = list( apply(expand.grid(c("Study"),c(1:nstudies.BS.df3.pseudo)),1, paste, collapse=" "),
                                                        c(1:length(fit.BS.df3.pseudo$coefficients))))


### Create empty matrix for the variance-covariance matrix of the coefficients

S.BS.df3.pseudo = matrix(NA, ncol=sum(c(1:length(fit.BS.df3.pseudo$coefficients))), nrow = nstudies.BS.df3.pseudo )
...
for( i in unique(df3.with.pseudo$Study)){
  ...
  # Fit the GAM
  fit = gam(formula = formula.BS.pseudo ,
            knots = Knots.BS.df3.pseudo, 
            weights = weight,
            family = binomial("logit"), data = minidf3.with.pseudo)
  ### Extract the coefficients and their standard errors for mvmeta
  estimated.coefficients.BS.df3.pseudo[j,] = fit$coefficients
  S.BS.df3.pseudo[j,] = vcov(fit)[lower.tri(vcov(fit), diag = T)]
...
#### Perform a multivariate meta-analysis
mv.fit.BS.df3.pseudo = mvmeta(estimated.coefficients.BS.df3.pseudo, S.BS.df3.pseudo)


prediction.interval.mvmeta.BS.df3.pseudo =  X.p.BS.df3 %*% coef(mv.fit.BS.df3.pseudo)

prediction.interval.mvmeta.lower.BS.df3.pseudo =  X.p.BS.df3 %*% coef(mv.fit.BS.df3.pseudo) - sqrt( rowSums(X.p.BS.df3  * (X.p.BS.df3  %*% vcov(mv.fit.BS.df3.pseudo)))) * qt(0.025, dim(df3)[1] - 3)

prediction.interval.mvmeta.upper.BS.df3.pseudo =  X.p.BS.df3 %*% coef(mv.fit.BS.df3.pseudo) + sqrt( rowSums(X.p.BS.df3  * (X.p.BS.df3  %*% vcov(mv.fit.BS.df3.pseudo)))) * qt(0.025, dim(df3)[1] - 3)


mvmeta.df3.pseudo.BS = cbind(df3[,c("Study","BMI","Treatment")],
                      fit =  expit(prediction.interval.mvmeta.BS.df3.pseudo), 
                      Lower= expit(prediction.interval.mvmeta.lower.BS.df3.pseudo),
                      Upper =expit(prediction.interval.mvmeta.upper.BS.df3.pseudo ))





g.mvmeta.total.BS.Comb.pseudo = ggplot(mvmeta.df3.pseudo.BS,aes(x = BMI, fit, linetype= Treatment, color= Treatment)) + 
  geom_line(size=2)+ ylim(c(0,1))+
  ylab("")+ xlab("") + scale_color_jama()+ 
  theme_bw()+ geom_ribbon(mapping = aes(ymin=Lower, ymax=Upper),alpha=0.25)+
  theme(plot.title    = element_text(hjust = 0.5,size = 26,face = "bold.italic"),
...
MV.meta_absolute_difference.RCS.Comb =  risk.diff.creator(dataframe = mvmeta.df2.RCS,
                                                        treatment = "Treatment",
...
MV.meta_absolute_difference.BS.Comb =  risk.diff.creator(dataframe = mvmeta.df2.BS,
                                                       treatment = "Treatment",
                                                       matching.variables=  c("BMI"),
                                                       outcome= NULL, 
                                                       predicted.outcome = "fit", 

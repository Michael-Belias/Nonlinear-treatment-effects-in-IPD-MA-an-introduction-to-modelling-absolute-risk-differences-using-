#### This is the script for the empirical example. 
...
Knots.NS.MV  = list (age = c(-9, -4, 0, 2.5 , 9.2, 13.5, 18))
...
#### Perform for each region a multivariate meta-analysis
mv.fit.NS.miniIPD= mvmeta(estimated.coefficients.miniIPD, S.miniIPD,control = list(maxiter=1000))


prediction.interval.mvmeta.miniIPD=  X.p.miniIPD%*% coef(mv.fit.NS.miniIPD)

prediction.interval.mvmeta.lower.miniIPD=  X.p.miniIPD%*% coef(mv.fit.NS.miniIPD) - sqrt( rowSums(X.p.miniIPD * (X.p.miniIPD  %*% vcov(mv.fit.NS.miniIPD)))) * 1.96

prediction.interval.mvmeta.upper.miniIPD =  X.p.miniIPD %*% coef(mv.fit.NS.miniIPD) + sqrt( rowSums(X.p.miniIPD  * (X.p.miniIPD  %*% vcov(mv.fit.NS.miniIPD)))) * 1.96



mvmeta.df.NS = cbind(new.dat[,c("study","age","treat","bilat_0")],
                     fit =  expit(prediction.interval.mvmeta.miniIPD), 
                     Lower= expit(prediction.interval.mvmeta.lower.miniIPD),
                     Upper =expit(prediction.interval.mvmeta.upper.miniIPD ))
...
g.mvmeta.total.NS = ggplot(mvmeta.df.NS,aes(x = age, fit, linetype= treat, color= treat)) + 
  geom_line(size=2)+ ylim(c(0,1))+
  ylab("")+ xlab("") + scale_color_jama()+ facet_wrap(~bilat_0)+
  theme_bw()+ geom_ribbon(mapping = aes(ymin=Lower, ymax=Upper),alpha=0.25)+
  theme(plot.title    = element_text(hjust = 0.5,size = 26,face = "bold.italic"),
...
p1= ggplot(mvmeta.df.NS,aes(x = age, fit, linetype= treat, color= treat)) + 
  geom_line(size=2)+ ylim(c(0,1))+
  ylab("")+ xlab("") + scale_color_jama()+ facet_wrap(~bilat_0)+
  theme_bw()+ geom_ribbon(mapping = aes(ymin=Lower, ymax=Upper),alpha=0.25)+
  theme(plot.title    = element_text(hjust = 0.5,size = 26,face = "bold.italic"),
        ...
        legend.title =element_blank(),
        legend.position = "bottom") + 
  geom_text(data= data.frame( label = c("", "a"),bilat_0   = mvmeta.df.NS$bilat_0[c(1,3)]),
            mapping = aes(x=0.3, y = 0.75, label = label), 
...
MV.meta_absolute_difference.NS=  risk.diff.creator(dataframe = mvmeta.df.NS,
                                                   treatment = "treat",
                                                   matching.variables=  c("study","age","bilat_0"),
                                                   outcome= NULL, 
                                                   predicted.outcome = "fit", 
...
MV.meta_absolute_difference.NS.plot = ggplot(MV.meta_absolute_difference.NS,aes(x = age, fit.diff)) +
  geom_line(size=2)+ facet_wrap(.~bilat_0)+
  ylab("")+ xlab("") + scale_color_jama()+ 
  theme_bw()+ 
  geom_ribbon(mapping = aes(ymin=diff.lower, ymax=diff.upper),alpha=0.25)+
...
grid.arrange(MV.meta_absolute_difference.RCS.plot, 
             MV.meta_absolute_difference.NS.plot,
             top = textGrob("", vjust = 1, gp = gpar(fontface = "bold", fontsize = 50)),
             left = textGrob("Antibiotics effect", rot = 90, vjust = 1, 
                             gp = gpar(fontface = "bold", fontsize = 32)),
...
Knots.NS  = list (age = c(-9.2, -4.6,  0,  4.6,  9.2, 13.7, 18.3))
...
fit.NS <- gam(formula = poutcome ~   treat + bilat_0 + age + treat*bilat_0 + age*treat + age*bilat_0 + 
                ...
                s(treat,by =  as.numeric(study),  bs="re")+
                s(bilat_0,by =  as.numeric(study),  bs="re"),
              family = binomial("logit"), data = miniIPD,knots = Knots.NS,
              method="REML")
